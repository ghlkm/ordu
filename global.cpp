...
bool MBRisValid(const int &Dimen, const float hs[], const float mbr[], vector<string> &Comb)
{
	int numAbove = 0;
	int numBelow = 0;
	int numOn = 0;
	long int numOfVertices = Comb.size();
	//long int numOfDim = Comb[0].size();
	...
	{
		vector<float> tmpPT(numOfDim, 0.0);
...
bool incomparableset(float* PG[], long int pi, long int pj, vector<float>& weight)
{
	...
	for (int i = 0; i < d-1; i++)
	{
		spi += PG[pi][i] * weight[i];
		spj += PG[pj][i] * weight[i];
		wd += weight[i];
